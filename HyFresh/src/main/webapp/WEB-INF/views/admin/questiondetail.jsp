...
<section class="py-5">
	<div class="container">
		...
		<table class="table table-hover">
			...
			<tr>
				<th colspan="2"><a href="/admin/questionlist?pageNum=${cri.pageNum}&amount=${cri.amount}"
					class="btn btn-success">목록</a> <a
					href="/question/remove?qno=${qvo.qno }"
					class="btn btn-outline-danger" id="delBtn">삭제</a></th>
			</tr>
		</table>
		<form action="/admin/questionremove" id="delForm" method="post">
...
<script>
	...
	function listAnswer(param_qno){
		  $.getJSON("/admin/list/"+param_qno+".json", function(aList){
		...
		function printListAnswer(listArr) {
		  ...
		  ulTag += '<li class="nav-item col-md-7">'+avo.content+'</li>';
		  ulTag += '<li class="nav-item col-md-2">'+displayTime(avo.regd8)+'</li>';
		  ulTag += '<button class="btn btn-outline-danger col-md-1 delBtn" type="button" data-ano="'+avo.ano+'">삭제</button></li></ul></div>'
